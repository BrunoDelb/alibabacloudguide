...
<!-- -->
```
-   \`Origin URL\`: select \`Add Prefix or Suffix\` and specify
    \`https\` and \`www.mywebsite.com,\`
...
-   \`Authorized RAM Role\`: this\`\` is the role to use: \`New RAM
    Role\` (a new RAM role is created) or \`AliyunOSSRole\` (if this
    role does not exist, it is created),
...
-   Enable or disable TLS versions (\`TLSv1.0\`, \`TLSv1.1\`,
    \`TLSv1.2\` or \`TLSv1.3\`).
...
status code. Both the request and the response contain the HTTP
\`Range\` header.
